...
def generate_and_save_images(model, epoch, datasets, summary_writer, img_dir, labels=None, **kwargs) -> float:
    ...
    if labels is None or len(labels)!=truths.shape[1]: labels=list(range(truths.shape[1]))

    if truths.shape[1]==8:
        for idx, ax, label in zip(list(range(truths.shape[1])), axs, labels):
            truth = truths[:, idx]
            prediction = predictions[:, idx]
            x_range = [np.min([truth.min(), prediction.min()]), np.max([truth.max(), prediction.max()])]
            yvals, _, _ = ax.hist(truth, bins=40, range=x_range, label='Truth', **config)
            max_y = np.max(yvals) * 1.1
            ax.hist(prediction, bins=40, range=x_range, label='Generator', **config)
            ax.set_xlabel(label)
            ax.set_ylim(0, max_y)
